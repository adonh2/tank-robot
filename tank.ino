...
void setup(){
  ...
  procedure(0);  //Servo rotates to the middle
...
void loop(){
  ...
  if (ir_rec == 0xFF22DD)   //Robot car turns left 0xFF22DD
  {
    Car_left();
    matrix_display(left);  //Display left-turning image
  }
  if (ir_rec == 0xFFC23D)   //Robot car turns right 0xFFC23D
  {
    Car_right();
  ...
  if (ir_rec == 0xFF30CF)   //robot car rotates anticlockwise 0xFF22DD
  {
    Car_left();
    matrix_display(left);  //show anticlockwise rotation picture
  }
  if (ir_rec == 0xFF7A85)  //robot car rotates clockwise 0xFF7A85
  {
    Car_right();
    matrix_display(right);  //show clockwise rotation picture
 }
 if (ir_rec == 0xFF9867)  //robot car says owners name
  {
    matrix_display(ADON01);  //dislplays ADON
    procedure(0);
 }
 if (ir_rec == 0xFF6897)  //robot car looks left
  {
    look_left();
 }
 if (ir_rec == 0xFFB04F)  //robot car looks right
  {
    look_right();
 }
}
...
void Car_front()
{
  ...
  analogWrite(MR_PWM,500);
  digitalWrite(ML_Ctrl,LOW);
  analogWrite(ML_PWM,500);
}
void Car_back()
{
